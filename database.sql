...
CREATE TABLE IF NOT EXISTS `payment_refunds` (
  ...
  `receiverBankBin` VARCHAR(10) DEFAULT NULL,
  ...
  `payosPayoutId` VARCHAR(100) DEFAULT NULL,
  `payosTransactionId` VARCHAR(100) DEFAULT NULL,
  `payosProviderState` VARCHAR(64) DEFAULT NULL,
  ...
  UNIQUE KEY `uq_payment_refunds_reference` (`referenceId`),
  UNIQUE KEY `uq_payment_refunds_idempotency` (`idempotencyKey`),
  KEY `idx_payment_refunds_status` (`statusId`, `requestedAt`),
  KEY `idx_payment_refunds_mode_status_updated` (`refundMode`, `statusId`, `updatedAt`),
  KEY `idx_payment_refunds_payos_payout` (`payosPayoutId`),
  KEY `idx_payment_refunds_payos_transaction` (`payosTransactionId`),
  ...
  CONSTRAINT `fk_payment_refunds_requested_by` FOREIGN KEY (`requestedBy`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_payment_refunds_approved_by` FOREIGN KEY (`approvedBy`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_payment_refunds_rejected_by` FOREIGN KEY (`rejectedBy`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
